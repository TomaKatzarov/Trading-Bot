...
class Position:
    ...
    def update(self, current_price: float) -> None:
        ...
        cost_basis = self.cost_basis if self.cost_basis != 0 else 1e-12
...
class PortfolioManager:
    ...
    def get_max_position_value(self, symbol: str) -> float:
        ...
        equity = self.get_equity()
        max_from_position_limit = equity * self.config.max_position_size_pct
    ...
    # Position management
    # ------------------------------------------------------------------
    def can_open_position(self, symbol: str, target_value: float) -> Tuple[bool, Optional[str]]:
        ...
        available_capital = self.get_available_capital()
        if target_value > available_capital + 1e-8:
        ...
        total_exposure_after = sum(position.current_value for position in self.positions.values()) + target_value
        leverage_after = total_exposure_after / equity if equity > 0 else float("inf")
        if leverage_after > self.config.max_leverage + 1e-8:
            return False, f"exceeds_leverage_limit_{self.config.max_leverage:.2f}"
    ...
    def open_position(
        ...
        can_open, reason = self.can_open_position(symbol, total_cost)
        if not can_open:
        ...
        position = Position(
            ...
            position_id=f"{symbol}_{entry_step}",
    ...
    def close_position(
        ...
        shares_to_close = position.shares if shares_to_close is None else shares_to_close
        ...
        cost_fraction = shares_to_close / position.shares
        ...
        if shares_to_close >= position.shares - 1e-8:
    ...
    def enforce_risk_limits(
        ...
        for violation in violations:
            if violation["type"] == "portfolio_drawdown":
                ...
                if symbol not in current_prices:
                    continue
                logger.warning("Position loss limit exceeded for %s. Auto-closing position.", symbol)
                _, trade = self.close_position(
                    symbol=symbol,
                    shares_to_close=None,
                    exit_price=current_prices[symbol],
